...
### Por que eles importam na era de IA

O papel dos fundamentos mudou — não acabou. Antes você estudava fundamentos para *produzir*. Agora estuda para *verificar*. A IA gera código; você precisa saber se o código que ela gerou presta. Sem julgamento fundamentado, você vira um gerente de código que não entende o que aprova.

### JVM e Runtime Java

- Gerenciamento de memória: heap, stack, metaspace
...
- Condições de corrida, deadlocks, livelocks
- Padrões: producer-consumer, thread pool, executors
- Programação reativa (conceito, não apenas framework): backpressure, streams não-bloqueantes
- Quando concorrência é solução e quando é problema
